...
def soc_experiment(base_dir='./soe_no_aug'):
    ...
    train_model(model, mode, base_dir, train_prefix, test_prefix, False, run_times)
...
def soc_reduce_sample_experiement(base_dir='./reduce', train_shapes=[88, 88]):
    ...
    for name in name_str:
        ...
        test_prefix = 'test'
        model = get_model(classes=10)
        train_model(model, mode, b_dir, train_prefix, test_prefix, True, run_times, train_shapes=train_shapes)
...
def eoc_vv_experiment(base_dir='./vv', train_shapes=[91, 91], save_best=1):
    ...
    train_model(model, mode, base_dir, train_prefix, test_prefix, False, run_times, train_shapes=train_shapes, save_best=save_best)
